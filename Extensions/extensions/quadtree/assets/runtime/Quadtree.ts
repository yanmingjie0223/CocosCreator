...
export class Quadtree<ObjectsType extends Rectangle | Circle | Line | Indexable> {
	...
	/**
	 * Remove an object into the node
	 *
	 * @example you can use any shape here (or object with a qtIndex method, see README):
	 * ```typescript
	 * const tree = new Quadtree({ width: 100, height: 100 });
	 * const rect = new Rectangle({ x: 25, y: 25, width: 10, height: 10, data: 'data' });
	 * tree.insert(rect);
	 * tree.rmove(rect);
	 * ```
	 *
	...
	retrieve(obj: Rectangle | Circle | Line | Indexable): ObjectsType[] {
		const indexes = this.getIndex(obj);
		let returnObjects: ObjectsType[] = [];
		returnObjects.push(...this.objects);

		//if we have subnodes, retrieve their objects
		if (this.nodes.length) {
			for (let i = 0; i < indexes.length; i++) {
				const nodeRetObjects = this.nodes[indexes[i]].retrieve(obj);
				returnObjects.push(...nodeRetObjects);
			}
		}
