...
export default class Agent {
	...
	computeNewVelocity() {
		...
		for (var i = 0; i < this.obstaclNeighbors.length; ++i) {
			...
			for (var j = 0; j < orcaLines.length; ++j) {
				if (
					RVOMath.det(
						relativePosition1.scale(invTimeHorizonObst).minus(orcaLines[j].point),
						orcaLines[j].direction
					) -
						invTimeHorizonObst * this.radius >=
						-RVOMath.RVO_EPSILON &&
					RVOMath.det(
						relativePosition2.scale(invTimeHorizonObst).minus(orcaLines[j].point),
						orcaLines[j].direction
					) -
						invTimeHorizonObst * this.radius >=
						-RVOMath.RVO_EPSILON
			...
			let s =
				relativePosition1.scale(-1).multiply(obstacleVector) /
				RVOMath.absSq(obstacleVector); //  (-relativePosition1 * obstacleVector) / RVOMath.absSq(obstacleVector)
			let distSqLine = RVOMath.absSq(
				relativePosition1.scale(-1).minus(obstacleVector.scale(s))
			); // RVOMath.absSq(-relativePosition1 - s * obstacleVector)
			...
			if (s < 0 && distSq1 <= radiusSq) {
				...
				if (obstacle1.isConvex) {
					...
					line.direction = RVOMath.normalize(
						new Vector2D(-relativePosition1.y, relativePosition1.x)
					);
					orcaLines.push(line);
				}
				continue;
				...
				if (obstacle2.isConvex && RVOMath.det(relativePosition2, obstacle2.unitDir) >= 0) {
					line.point = new Vector2D(0, 0);
					line.direction = RVOMath.normalize(
						new Vector2D(-relativePosition2.y, relativePosition2.x)
					);
					orcaLines.push(line);
				}
				continue;
			...
			if (
				obstacle1.isConvex &&
				RVOMath.det(leftLegDirection, leftNeighbor.unitDir.scale(-1)) >= 0.0
			) {
			...
			let t =
				obstacle1 == obstacle2
					? 0.5
					: this.velocity.minus(leftCutoff).multiply(cutoffVec) /
			...
			let distSqLeft =
				tLeft < 0.0
					? Infinity
					: RVOMath.absSq(
							this.velocity.minus(leftLegDirection.scale(tLeft).plus(leftCutoff))
					  );
			let distSqRight =
				tRight < 0.0
					? Infinity
					: RVOMath.absSq(
							this.velocity.minus(rightLegDirection.scale(tRight).plus(rightCutoff))
					  );
	...
	insertObstacleNeighbor(obstacle: Obstacle, rangeSq: number) {
		...
		let distSq = RVOMath.distSqPointLineSegment(
			obstacle.point,
			nextObstacle.point,
			this.position
		);
	...
	private _linearProgram1(
		...
		for (var i = 0; i < lineNo; ++i) {
			...
			var numerator = RVOMath.det(
				lines[i].direction,
				lines[lineNo].point.minus(lines[i].point)
			);
	...
	private _linearProgram3(
		...
		for (var i = beginLine; i < lines.length; ++i) {
			if (
				RVOMath.det(lines[i].direction, lines[i].point.minus(this._newVelocity)) > distance
			) {
				...
				for (var j = numObstLines; j < i; ++j) {
					...
					if (Math.abs(determinant) <= RVOMath.RVO_EPSILON) {
						...
						var aux = lines[i].direction.scale(
							RVOMath.det(lines[j].direction, lines[i].point.minus(lines[j].point)) /
								determinant
						);
					...
					line.direction = RVOMath.normalize(
						lines[j].direction.minus(lines[i].direction)
					);
					projLines.push(line);
				}
				...
				var tempResult = this._newVelocity;
				if (
					this._linearProgram2(
						projLines,
						radius,
						new Vector2D(-lines[i].direction.y, lines[i].direction.x),
						true
					) < projLines.length
				) {
