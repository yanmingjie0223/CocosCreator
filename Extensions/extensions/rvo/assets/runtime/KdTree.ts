...
export default class KdTree {
	...
	private obstacleTree: ObstacleTreeNode | null = new ObstacleTreeNode();
	...
	private _buildAgentTreeRecursive(begin: number, end: number, node: number) {
		...
		for (var i = begin + 1; i < end; ++i) {
			...
			this.agentTree[node].minX = Math.max(
			...
			this.agentTree[node].minY = Math.max(
		...
		if (end - begin > this.MAXLEAF_SIZE) {
			...
			var splitValue = isVertical
				? 0.5 * (this.agentTree[node].maxX + this.agentTree[node].minX)
				: 0.5 * (this.agentTree[node].maxY + this.agentTree[node].minY);
			...
			while (left < right) {
				while (
					left < right &&
					(isVertical ? this.agents[left].position.x : this.agents[left].position.y) <
						splitValue
				) {
					++left;
				}
	...
	private _buildObstacleTreeRecursive(obstacles: Array<Obstacle>): ObstacleTreeNode | null {
		if (obstacles.length == 0) {
			...
			for (let i = 0, len = obstacles.length; i < len; ++i) {
				...
				for (let j = 0; j < obstacles.length; j++) {
					...
					let j1LeftOfI = RVOMath.leftOf(
						obstacleI1.point,
						obstacleI2.point,
						obstacleJ1.point
					);
					let j2LeftOfI = RVOMath.leftOf(
						obstacleI1.point,
						obstacleI2.point,
						obstacleJ2.point
					);
			...
			{
				/* Build split node. */
				let leftObstacles: Array<Obstacle> = [];
				for (var n = 0; n < minLeft; ++n) leftObstacles.push(null!);

				let rightObstacles: Array<Obstacle> = [];
				...
				for (var j = 0; j < obstacles.length; ++j) {
					...
					if (j1LeftOfI >= -RVOMath.RVO_EPSILON && j2LeftOfI >= -RVOMath.RVO_EPSILON) {
						...
						let t =
							RVOMath.det(
								obstacleI2.point.minus(obstacleI1.point),
								obstacleJ1.point.minus(obstacleI1.point)
							) /
							RVOMath.det(
								obstacleI2.point.minus(obstacleI1.point),
								obstacleJ1.point.minus(obstacleJ2.point)
							);

						var splitpoint = obstacleJ1.point.plus(
							obstacleJ2.point.minus(obstacleJ1.point).scale(t)
						);
	...
	private _queryObstacleTreeRecursive(agent: Agent, rangeSq: number, node: ObstacleTreeNode | null) {
		if (node == null) {
			...
			this._queryObstacleTreeRecursive(
				agent,
				rangeSq,
				agentLeftOfLine >= 0 ? node.left : node.right
			);

			let distSqLine =
				RVOMath.sqr(agentLeftOfLine) /
				RVOMath.absSq(obstacle2.point.minus(obstacle1.point));
			...
			if (distSqLine < rangeSq) {
				...
				this._queryObstacleTreeRecursive(
					agent,
					rangeSq,
					agentLeftOfLine >= 0 ? node.right : node.left
				);
			}
		}
	}
	...
	private _queryVisibilityRecursive(
		q1: Vector2D,
		q2: Vector2D,
		radius: number,
