 ...
 * @Last Modified time: 2019-01-24 21:26:06
 */
export default class BaseView extends fgui.GComponent {
    ...
    public set contentPane(_contentPane: fgui.GComponent) {
        ...
        this.setSize(AppConfig.initWidth, AppConfig.initHeight);
    ...
    private initStart(): void {
        ...
        if (!this._isInit || !isExistPkg) {
            App.ResManager.loadPackage(this._pkgName, this.toInitUI, this.destroy, this);
        }
        else {
            this.onCompleteUI();
        }
    ...
    private onCompleteAnimation(): void {
        // 背景蒙层点击事件
        if (this._bgLoader) {
            this._bgLoader.offClick(this.onClickBG, this);
            this._bgLoader.onClick(this.onClickBG, this);
        }
        // 完全显示方法，给继承子类用
        this.onShown();
    }
        ...
        if (this.type === ViewType.VIEW) {
            ...
            if (!this._bgLoader) {
                ...
                if (this.type === ViewType.X_WINDOW) {
                    this._bgLoader.url = 'ui://b3udd8jiadmq3';
